    ...
    def leave_FunctionDef(
        ...
        if param_types.split(",")[0].strip() == "...":
            new_returns = cst.Annotation(
                annotation=cst.parse_expression(return_type.strip())
            )
...
class FunctionParamTypeCommentTransformer(cst.CSTTransformer):
    ...
    def visit_Comment(self, node: cst.Comment) -> None:
        ...
        if type_comment_match:
            self.type_comments.append(
                cst.parse_expression(type_comment_match.group(1).strip())
            )
...
class AssignTypeCommentTransformer(cst.CSTTransformer):
    ...
    def visit_Comment(self, node: cst.Comment) -> None:
        ...
        if type_comment_match and self.pending_assignment:
            ...
            annotation = cst.Annotation(
                annotation=cst.parse_expression(type_comment_match.group(1).strip())
            )
    ...
    def leave_SimpleStatementLine(
        ...
        if self.pending_assignment:
            new_body = [
                self.pending_assignment if isinstance(item, cst.Assign) else item
